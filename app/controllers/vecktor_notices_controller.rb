class VecktorNoticesController < ApplicationController
  ...
  def index
    @vecktor_notices = VecktorNotice.all
  ...
  def create
    ...
    respond_to do |format|
      if @vecktor_notice.save
        format.html { redirect_to @vecktor_notice, notice: 'Vecktor notice was successfully created.' }
  ...
  def update
    ...
    respond_to do |format|
      if @vecktor_notice.update_attributes(params[:vecktor_notice])
        format.html { redirect_to @vecktor_notice, notice: 'Vecktor notice was successfully updated.' }
